...
- 视频：MiniMax Video / Kling / Seedance
- 配音：MiniMax
- 拼接：FFmpeg
...
| 视频片段生成 | MiniMax Video | `MiniMax-Hailuo-2.3-Fast`, `MiniMax-Hailuo-2.3`, `T2V-01-Director` |
...
这版 release 镜像会刻意做轻量化：
- 优先服务于 脚本 -> 审核 -> 关键帧 -> 配音 -> 视频片段 -> 组装 这条主流程
- 默认不把本地 Whisper / Torch 这类重依赖打进镜像
- 像 pyJianYingDraft 这种可选集成，缺失时会自动回退

### 直接使用预编译镜像

带 tag 的 release 可以通过 GitHub Actions 自动发布到 GHCR，用户可以直接运行：
...
说明：
- 这是一版更偏运行时的轻量镜像，不是完整开发镜像
- 如果你确实需要把本地 ASR / Whisper 能力一起打进 Docker，建议在此基础上自己扩展
- 对大多数用户来说，只要通过浏览器完成配置，并接好外部 provider，就已经可以直接开始生成视频
